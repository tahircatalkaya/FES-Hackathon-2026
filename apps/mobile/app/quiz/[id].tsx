...
export default function Quiz() {
  ...
  return (
    <Screen tabBar={false}>
      <Header title={ch.title} subtitle={`Lernen mit ${chameleonName} · Kapitel ${CHAPTERS.indexOf(ch) + 1}/${CHAPTERS.length}`} color={col} />
          ...
          <Card>
              ...
              {q.options.map((o, i) => {
                ...
                return (
                  <Pressable key={i} disabled={reveal} onPress={() => { setPicked(i); haptic(i === q.answer ? 'success' : 'warn'); }} style={{ padding: 14, borderRadius: 16, borderWidth: 2, borderColor: reveal ? (isRight ? C.success : chosen ? C.danger : C.line) : C.line, backgroundColor: reveal && isRight ? C.success + '14' : '#fff' }}>
                    <Text style={[T.body, { fontWeight: '700', color: C.ink }]}>{String.fromCharCode(65 + i)} · {o}</Text>
                  </Pressable>
                );
              })}
            </View>
            {picked !== null && (
              <Animated.View entering={FadeInDown} style={{ marginTop: 12, backgroundColor: C.bg, borderRadius: 14, padding: 12 }}>
                <Text style={[T.label, { color: picked === q.answer ? C.success : C.danger }]}>{picked === q.answer ? 'Richtig' : 'Nicht ganz'}</Text>
                <Text style={[T.body, { marginTop: 4 }]}>{q.why}</Text>
                <View style={{ marginTop: 10 }}><Button label={step + 1 >= ch.questions.length ? 'Kapitel abschließen' : 'Weiter'} color={col} onPress={next} /></View>
              </Animated.View>
            )}
          </Card>
        </Animated.View>
      ...
      {done && (
        <Appear>
          <Card style={{ alignItems: 'center', paddingVertical: 24 }}>
            ...
            <Row style={{ marginTop: 6, gap: 6 }}><Text style={T.small}>2 Punkte je richtige Antwort, max 3 Kapitel/Tag</Text></Row>
