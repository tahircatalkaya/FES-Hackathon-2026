...
import { Button, Pill, T, haptic } from '@/components/ui';
import { C, CONTEXT, S } from '@/theme';
import { LANGS } from '@/i18n';
import { useT } from '@/i18n/useT';
import { useStore } from '@/store';

const SLIDES: { ctx: keyof typeof CONTEXT; t: 'onb.1.title' | 'onb.2.title' | 'onb.3.title'; b: 'onb.1.body' | 'onb.2.body' | 'onb.3.body'; emoji: string }[] = [
  { ctx: 'home', t: 'onb.1.title', b: 'onb.1.body', emoji: '🏙️' },
  { ctx: 'mobility', t: 'onb.2.title', b: 'onb.2.body', emoji: '🔍' },
  { ctx: 'food', t: 'onb.3.title', b: 'onb.3.body', emoji: '🔒' },
];
    ...
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      <ScrollView contentContainerStyle={{ paddingTop: insets.top + 12, paddingHorizontal: S.xl, paddingBottom: insets.bottom + 24, flexGrow: 1, maxWidth: 560, width: '100%', alignSelf: 'center' }} keyboardShouldPersistTaps="handled">
        ...
        <Animated.View key={i} entering={FadeInDown.springify().damping(16)} exiting={FadeOut} style={{ marginTop: 24, minHeight: 190 }}>
          {!last ? (
            <>
              <Text style={{ fontSize: 40 }}>{SLIDES[i].emoji}</Text>
              <Text style={[T.h1, { marginTop: 8 }]}>{t(SLIDES[i].t)}</Text>
              ...
              {i === 2 && (
                <Pressable onPress={() => setConsent(!consent)} style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginTop: 18, backgroundColor: '#fff', padding: 14, borderRadius: 16 }}>
                  ...
                  <Text style={[T.body, { flex: 1 }]}>Standort nur während bewusst gestarteter Fahrten. Aggregierte, anonyme Zahlen dürfen ins Frankfurt-Ziel einfließen (k ≥ 5).</Text>
                </Pressable>
              )}
            </>
          ) : (
            <>
              ...
              <TextInput value={name} onChangeText={setName} placeholder="Anzeigename, kein Klarname nötig" placeholderTextColor={C.muted} style={{ marginTop: 14, backgroundColor: '#fff', borderRadius: 16, padding: 16, fontSize: 18, fontWeight: '700', color: C.ink, borderWidth: 2, borderColor: color + '55' }} autoFocus maxLength={24} />
              <Text style={[T.small, { marginTop: 10 }]}>Öffentlich sichtbar ist nur dieser Name. Keine E-Mail, keine Telefonnummer, keine Adresse.</Text>
            </>
          )}
        </Animated.View>
        <View style={{ flex: 1 }} />
        ...
        <Button color={color} label={last ? t('onb.start') : t('common.next')} icon={last ? '🦎' : undefined} disabled={last && name.trim().length < 2} onPress={() => {
