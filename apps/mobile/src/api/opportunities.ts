...
export async function getOpportunities(lat: number, lon: number, radiusKm = 3): Promise<OppResult> {
  ...
  for(const item of items.filter(i=>i.id.startsWith('fsp-'))) {const report=shelves.find(r=>`fsp-${r.pointId}`===item.id);if(report)item.sub+=` · ${report.fill}, gemeldet ${new Date(report.at).toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit'})}`;}
  for (const b of baskets.items) items.push({ id: `bk-${b.id}`, layer: 'food', ctx: 'food', partner: 'foodsharing', title: b.title, sub: `Korb · bis ${b.expires_at ? new Date(b.expires_at).toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' }) : '–'}`, lat: b.lat, lon: b.lon, distance_m: d(b.lat, b.lon), icon: 'basket', emoji: '🧺', href: `/korb/${b.id}`, availability: b.status === 'available' ? 'offen' : 'reserviert', source: baskets.source, verification: baskets.source === 'api' ? 'API' : 'Simuliert' });
  ...
  for (const v of vytal.items) items.push({ id: `vy-${v.id}`, layer: 'reuse', ctx: 'reuse', partner: 'vytal', title: v.name, sub: `${v.address} · ${v.type.replace(/_/g, ' ').toLowerCase()}`, lat: v.lat, lon: v.lon, distance_m: d(v.lat, v.lon), icon: 'cafe', emoji: '🥡', href: `/mehrweg?store=${v.id}`, availability: 'jetzt', source: vytal.source, verification: vytal.source === 'api' ? 'API' : 'Simuliert' });
