...
export default function ReuseInventory({returnOnly=false}:{returnOnly?:boolean}) {
  ...
  return <View style={{gap:12}}>
    ...
    <Card style={{backgroundColor:C.reuse,gap:8}}><Text style={[T.h2,{color:'#fff'}]}>{returnOnly?'Deine Rückgabe':`${open.length} unterwegs · ${returned.length} zurück`}</Text><Text style={[T.body,{color:'#fff'}]}>Behälter abgeben. QR vom Personal scannen oder vier Ziffern eingeben. Dein Beleg wird danach sofort angezeigt.</Text></Card>
...
function LoanCard({loan:l,onSaved}:{loan:ReuseLoan;onSaved:()=>Promise<void>}) {
  ...
  return <Card style={{gap:10,borderLeftWidth:4,borderLeftColor:l.damage?C.warn:C.reuse}}>
    ...
    <Button label="QR scannen oder 4 Ziffern eingeben" icon="qr-code" color={C.reuse} onPress={()=>router.push({pathname:'/scan',params:{mode:'vytal-return',id:l.id}})}/>
