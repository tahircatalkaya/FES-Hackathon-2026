...
export default function ReuseInventory() {
  const t = useT();
  ...
  return <View style={{gap:12}}>
    ...
    {!ready?<Text style={T.body}>{t('components.reuse.loading')}</Text>:!signed?<TrustAccount color={C.reuse} onReady={()=>void load()}/>:<>
      ...
      {<Button label={t('components.reuse.borrow')} icon="scan" color={C.reuse} onPress={()=>router.push('/scan?mode=vytal')}/>}
