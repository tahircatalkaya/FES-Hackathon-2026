...
export default function PartnerLogo({ id, name }: { id: string; name: string }) {
  const t = useT();
  return <View accessible accessibilityLabel={t('components.partner.logo', { name })} style={{ width: 168, height: 82, borderRadius: 16, backgroundColor: id === 'transdev' ? '#C8102E' : id === 'main-lastenrad' ? '#46762F' : '#fff', alignItems: 'center', justifyContent: 'center', padding: 12 }}>
