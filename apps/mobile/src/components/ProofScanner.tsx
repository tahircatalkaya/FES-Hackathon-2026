...
export default function ProofScanner({kind,id}:{kind:'food'|'return'|'shelf';id:string}) {
  ...
  async function load(){setError('');try{const has=await trust.hasSession();setSigned(has);if(!has)return;
    if(kind==='food'){const h=(await trust.handoffs()).find(h=>h.id===id&&h.role==='provider');if(!h)throw new Error(t('components.scanner.pickupMissing'));setHandoff(h);}
  ...
  async function complete(){if(busy||!valid)return;setBusy(true);setError('');try{
    if(kind==='food'){
      ...
      setDone(t('components.scanner.handoffDone', { name: result.counterpart.name }));
  ...
  return <Screen tabBar={false}><Header title={kind==='food'?t('components.scanner.pickupTitle'):kind==='shelf'?'Regalmeldung bestätigen':t('components.scanner.returnTitle')} subtitle={kind==='food'?t('components.scanner.pickupSub'):kind==='shelf'?'QR scannen oder vier Ziffern eingeben':t('components.scanner.returnSub')} color={color}/>
