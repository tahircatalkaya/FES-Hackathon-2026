...
try:
    with sync_playwright() as p:
        ...
        E = lambda s: page.get_by_text(s, exact=True).first
        BACK = lambda: page.get_by_label('Zurück').first.click()
        # Fairteiler: drei Aktionen, je manuell eintragen (Foto/Sprache brauchen Kamera/Mikro)
        page.goto(B + '/fairteiler/6', wait_until='networkidle'); time.sleep(2)
        step(page, 'ft_shelf_open', lambda: E('Regal melden').click())
        step(page, 'ft_shelf_manual', lambda: E('Eintragen').click())
        step(page, 'ft_shelf_add', lambda: (page.get_by_placeholder('Was?').fill('Brötchen'), page.get_by_placeholder('Menge').fill('8 Stück'), E('Posten hinzufügen').click()))
        step(page, 'ft_shelf_back', BACK)
        step(page, 'ft_shelf_manual2', lambda: E('Eintragen').click())
        step(page, 'ft_shelf_add2', lambda: (page.get_by_placeholder('Was?').fill('Äpfel'), page.get_by_placeholder('Menge').fill('1 kg'), E('Posten hinzufügen').click(), E('voll').click(), T('Weiter').click()))
        step(page, 'ft_shelf_done', lambda: E('Stimmt so').click())
        step(page, 'ft_toast_close', lambda: T('Weiter').click())
        step(page, 'ft_hold', lambda: T('30 min halten').click())
        step(page, 'ft_stock_open', lambda: E('Einstellen').click())
        step(page, 'ft_stock_photo', lambda: E('Foto').click())
        step(page, 'ft_stock_photo_back', BACK)
        step(page, 'ft_stock_voice', lambda: E('Sprache').click())
        step(page, 'ft_stock_close', lambda: page.get_by_label('Schließen').first.click())
        step(page, 'ft_pickup_open', lambda: E('Abholen').click())
        step(page, 'ft_pickup_manual', lambda: (E('Eintragen').click(), page.get_by_placeholder('Was?').fill('Joghurt'), E('Posten hinzufügen').click(), T('Weiter').click(), E('Stimmt so').click()))
